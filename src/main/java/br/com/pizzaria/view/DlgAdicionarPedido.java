...
public class DlgAdicionarPedido extends JDialog {
	...
	/*private boolean testa(Cliente cliente , String linha ) {
		
		if (cliente.getText().trim().length() == 0 ){
		JOptionPane.showMessageDialog(botaoOk, "Necessário preencher o cliente" );
		return true;
	}
		return false;
}*/

	private boolean testaVazio(JTextField linha, String tamanho) {
		
		if (linha.getText().trim().length() == 0) {
			JOptionPane.showMessageDialog(botaoOk, "Necessário preencher " + tamanho);
			return true;
		}
		return false;
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if( linhaSelecionada < 0) {
					JOptionPane.showMessageDialog( botaoRemoverPizza, "Para remover uma pizza, selecione uma linha na lista de pizzas."); 
					return;
				}
	...
	private class ActionRemoverBebida implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if( linhaSelecionada < 0) {
					JOptionPane.showMessageDialog( botaoRemoverBebida, "Para remover uma bebida, selecione uma linha na lista de bebidas."); 
					return;
				}
