...
public class DlgAdicionarBebida extends JDialog {
	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (testaIdBebida(textQuantidade)) {
				return;
			}

			pressionouOk = true;

			PedidoBebida pedidoBebida = new PedidoBebida ((Bebida) comboCardapio.getSelectedItem(),
											Integer.parseInt(textQuantidade.getText().trim()));
	...
	private boolean testaIdBebida(JTextField textQuantidade) {
		int bebida = Integer.parseInt(textQuantidade.getText().trim());
		
		if (bebida <= 0) {
			   JOptionPane.showMessageDialog(botaoOk, "O número da bebida é igual ou menor que zero, não é permitido");
			   return true;
	}
